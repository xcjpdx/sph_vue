    ...
    <form @submit.prevent>
      <div class="register">
        ...
        <div class="content">
          ...
          <button @click="sendCode" class="codeStyle" ref="code">
            获取验证码
          </button>
        </div>
        <!-- 登录密码 -->
    ...
    async register() {
      ...
      if (
        ...
        this.$message.warning(
          "请输入完整的注册信息!或者检查是否已经勾选协议条款!"
        );
      }
    },
    //登录
    async login() {
      let { phone, password, isSelected } = this;
      //发送请求
      try {
        await this.$store.dispatch("login", { phone, password, isSelected });
        // 跳转到主页
        this.$router.push("/");
      } catch (error) {
